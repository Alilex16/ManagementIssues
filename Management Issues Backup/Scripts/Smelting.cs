...
public class Smelting : MonoBehaviour, IDropHandler
{
    ...
    private int smeltingOre; //unused yet
    ...
    void StartSmelting()
    {
        ...
        //copy.GetComponentInParent<RectTransform>();

        copy.transform.localRotation = Quaternion.identity;
        copy.transform.localScale = Vector3.one;
        copy.transform.localPosition = new Vector3(0, 0, 0);

        if (beingSmelted == 3 || beingSmelted == 11) //if Coal/Uranium - cannot be smelted
        {
            Debug.Log("Ore Number: " + Ores.oreNumberStatic);
            //Destroy(transform.GetChild(0).gameObject);
            transform.GetComponent<Image>().color = Color.red;
            toggleButton.GetComponent<Toggle>().interactable = false;
        }
        else
        {
            transform.GetComponent<Image>().color = colorOriginal;
    ...
    public void SmeltRocks()
    {
        ...
        int randomIngot = Random.Range(1, 8);
        
        if (randomIngot == 1)
        {
            PlayerStats.ingotIronAmount += 1;
        }
        if (randomIngot == 2)
        {
    ...
    void Update()
    {
        if (toggleButton.GetComponent<Toggle>().isOn == true && transform.childCount > 0)
        {
            ...
            smeltingBarImage.fillAmount = elapsedFloat / smeltingSpeed;
